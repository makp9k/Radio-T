...
class BackgroundPlayerService : Service() {
    ...
    private fun createNotification(news: Optional<NewsItem>): Notification {
        val builder = NotificationCompat.Builder(this, NotificationUtils.NotificationChannelInfo.PRIMARY.id)
                ...
                .addAction(
                        ...
                        NotificationIntentReceiver.getStreamStopIntent(this)
                )
    ...
    class NotificationIntentReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent?) {
           RadioTApplication.getAppComponent(context).streamPlayer().stop()
        }

        companion object {
            fun getStreamStopIntent(context: Context): PendingIntent? {
                return PendingIntent.getBroadcast(
                        context,
                        0,
                        Intent(context, NotificationIntentReceiver::class.java),
